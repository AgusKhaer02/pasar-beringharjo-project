...
$routes->post('admin/auth/authenticate', 'backend\AdminAuth::authenticate'); 

$routes->group('admin', ['filter' => 'adminAuth'] ,function ($route) {
    $route->group('auth', function ($route) {
        $route->get('/', 'backend\AdminAuth::login'); 
        $route->get('login', 'backend\AdminAuth::login'); 
        $route->get('logout', 'backend\AdminAuth::logout');
        // $route->get('adminInfo', 'backend\AdminAuth::adminInfo');
    });
    $route->group('denah', function ($route) {
        $route->get('/', 'backend\Denah::index');
        $route->group('los', function ($routes) {
            $routes->get('new-los', 'backend\Denah::formInsert');
            $routes->post('insert', 'backend\Denah::insert');

            $routes->get('update-los/(:any)', 'backend\Denah::formUpdate/$1');
            $routes->put('update', 'backend\Denah::update');
    
            $routes->delete('delete-post/(:any)', 'backend\Denah::delete/$1');
        });
    });
